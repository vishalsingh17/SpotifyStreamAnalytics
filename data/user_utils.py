...
class UserUtils:
    '''Utility class for generating random user data'''

    @staticmethod
    def generate_users_data(location_mapping_path: str, n_users: int) -> pd.DataFrame:
        '''
        Generate random users demographic data
        ...
        # randomly generate first name, last name, gender and location
        genders = [random.choice(['male', 'female']) for _ in range(n_users)]
        
        user_df["first_names"] = [indian_names.get_first_name(gender=gender) for gender in genders]
        
        user_df["last_names"] = [indian_names.get_last_name() for _ in range(n_users)]
        
        user_df["gender"] = genders

        location_idx = [random.randint(0, len(location_df)-1) for _ in range(n_users)]
        locations = location_df.iloc[location_idx].reset_index(drop=True)
        user_df = user_df.join(locations)

        return user_df

    @staticmethod
    def generate_dates_df(start_date: str, end_date: str) -> pd.DataFrame:
        '''
        ...
        # get timestamp at the start of each day
        dates_df["start_ts"] = dates_df["date"].values.astype(float)
        dates_df["start_ts"] = dates_df["start_ts"].astype(int)
        # get timestamp at the end of each day by adding 86k seconds (24 hours)
        dates_df["end_ts"] = dates_df["start_ts"] + 86399

        return dates_df

    @staticmethod
    def generate_user_song_interactions(users_df: pd.DataFrame,
